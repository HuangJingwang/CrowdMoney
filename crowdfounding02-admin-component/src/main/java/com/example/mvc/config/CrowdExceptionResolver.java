...
import com.example.util.judgeResult;
...
// 只是指定当前异常对应的页面即可
        ...
        System.out.println("此页面有错误");
        return commonResolveException(exception, request, response, viewName);
    }
    @ExceptionHandler(value = LoginAccountAlreadyInUseForUpdateException.class)
    public ModelAndView resolveLoginFailedForUpdateException(
            Exception exception, HttpServletRequest request,
            HttpServletResponse response
    ) throws IOException {
// 只是指定当前异常对应的页面即可
        String viewName = "admin_edit";
        return commonResolveException(exception, request, response, viewName);
    }
...
// 2.如果是 Ajax 请求
        if(judgeResult.judgeRequestType(request)) {
// 3.从当前异常对象中获取异常信息
